// worker.index.js (versão ajustada ao schema e melhorias)
...
/* ---------- Likes for Instagram posts (RapidAPI) ---------- */
...
async function processBatch() {
  ...
  const query = {
    status: "pendente",
    tipo_acao: "curtir",
    rede_social: { $in: ["Instagram", "instagram"] }
  };
          ...
          if (!isNaN(valor) && valor > 0) {
            ...
            if (action.id_pedido && SMM_API_KEY) {
              try {
                ...
                const resp = await axios.post("https://smmsociais.com/api/incrementar-validadas", payload, { headers: { Authorization: `Bearer ${SMM_API_KEY}`, "Content-Type": "application/json" }, timeout: 10000 });
              ...
              if (!action.id_pedido) console.log("   → Pulando notificação smmsociais: action.id_pedido ausente");
              else if (!SMM_API_KEY) console.log("   → Pulando notificação smmsociais: SMM_API_KEY não configurada");
            }
          } else {
...
async function processLikesBatch() {
  ...
  console.log("🔎 Executando find (curtir) com query:", JSON.stringify(query));
  const acoes = await colecao.find(query).sort({ data: 1 }).limit(MAX_BATCH).toArray();
...
/* ---------- main loop: process both (seguir) e (curtir) ---------- */
